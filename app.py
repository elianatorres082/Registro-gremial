...
SMATA_FDP_HISTORICO = [
    ...
    {"legajo":"717",  "nombre":"Karg Miguel",           "cargo_gremial":"Comisión Directiva",                                  "desde":"2011", "status":"Activo",               "detalle":""},
...
def _render_seccion_externa(titulo, emoji, data_file, padron_file, key_prefix):
    ...
    with tab_rep:
        ...
        if df_rep.empty:
            ...
            _desvios_lower_ext = [d.strip().lower() for d in DESVIOS]
            df_rf["_es_desvio"] = df_rf["Movilidad Gremial x Semana x Dia"].str.strip().str.lower().isin(_desvios_lower_ext)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if df_all.empty:
        ...
        _desvios_lower = [d.strip().lower() for d in DESVIOS]
        df_f["_es_desvio"] = df_f["Movilidad Gremial x Semana x Dia"].str.strip().str.lower().isin(_desvios_lower)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if df_all.empty:
        ...
        _desvios_lower = [d.strip().lower() for d in DESVIOS]
        df_d["_es_desvio"] = df_d["Movilidad Gremial x Semana x Dia"].str.strip().str.lower().isin(_desvios_lower)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_edit:
        ...
        STATUS_OPTS = ["Activo", "Desvinculado", "Reincorporado a TASA", "Cargo Político"]
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("""
    Genera automáticamente el reporte completo en PPT con los datos actuales del registro.
    El archivo incluye los mismos 13 slides del reporte manual:
    portada, indicadores clave, tendencia mensual, desvíos por sector, motivos por sector,
    desvíos por turno, top 5 delegados, análisis focalizado, motivos globales vs top 5,
    análisis LLT, detalle LLT, lectura del problema y conclusiones.
    """)

    df_all = load_data(_mtime=_get_mtime(DATA_FILE))

    if df_all.empty:
        st.warning("No hay registros cargados. Importá el histórico primero.")
    else:
        df_all["_fecha"] = _parsear_fechas(df_all)
        fechas_validas = df_all["_fecha"].dropna()
        ...
        if fechas_validas.empty:
            ...
            fc1, fc2 = st.columns(2)
            with fc1:
                ppt_desde = st.date_input("Período desde", value=fecha_min_data, format="DD/MM/YYYY", key="ppt_desde")
            with fc2:
                ppt_hasta = st.date_input("Período hasta", value=fecha_max_data, format="DD/MM/YYYY", key="ppt_hasta")

            # Preview de lo que va a incluir
            df_prev = df_all[
                (df_all["_fecha"] >= pd.Timestamp(ppt_desde)) &
                (df_all["_fecha"] <= pd.Timestamp(ppt_hasta))
            ].copy()
            _desvios_lower_ppt = [d.strip().lower() for d in DESVIOS]
            df_prev["_es_desvio"] = df_prev["Movilidad Gremial x Semana x Dia"].str.strip().str.lower().isin(_desvios_lower_ppt)

            if not df_prev.empty:
                n_dev_prev = int(df_prev["_es_desvio"].sum())
                tasa_prev  = round(n_dev_prev / len(df_prev) * 100, 1) if len(df_prev) > 0 else 0
                p1,p2,p3 = st.columns(3)
                p1.markdown(f'<div class="kpi-card"><div class="kpi-label">Registros en el período</div><div class="kpi-value">{len(df_prev)}</div></div>', unsafe_allow_html=True)
                p2.markdown(f'<div class="kpi-card danger"><div class="kpi-label">Desvíos</div><div class="kpi-value">{n_dev_prev}</div></div>', unsafe_allow_html=True)
            ...
            if st.button("🖨️ Generar PPT", type="primary", use_container_width=True, disabled=df_prev.empty):
                with st.spinner("Generando reporte... esto tarda unos segundos"):
                    import subprocess, tempfile, sys

                    # Guardar CSV temporal con el período filtrado (sin columna _fecha/_es_desvio)
                    df_export = df_prev.drop(columns=["_fecha","_es_desvio"], errors="ignore")
                    tmp_csv = os.path.join(DATA_DIR, "_tmp_reporte.csv")
                    df_export.to_csv(tmp_csv, index=False)

                    # Buscar el script js
                    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "generar_reporte.js")
                    if not os.path.exists(script_path):
                        # fallback: mismo directorio
                        script_path = os.path.join(DATA_DIR, "generar_reporte.js")

                    out_pptx = os.path.join(DATA_DIR, f"Movilidad_Gremial_{ppt_desde.strftime('%d%m%Y')}_{ppt_hasta.strftime('%d%m%Y')}.pptx")

                    try:
                        result = subprocess.run(
                            ["node", script_path, tmp_csv,
                             ppt_desde.strftime("%d/%m/%Y"), ppt_hasta.strftime("%d/%m/%Y")],
                        ...
                        if result.returncode == 0 and os.path.exists(out_pptx):
                            ...
                            st.download_button(
                                ...
                                data=ppt_bytes,
                                file_name=os.path.basename(out_pptx),
                                mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                                use_container_width=True, type="primary"
                            )
                        else:
                            st.error(f"Error al generar el PPT.")
                            if result.stderr:
                                st.code(result.stderr[:500])
                    except subprocess.TimeoutExpired:
                        st.error("Tiempo de espera agotado. Intentá con un período más corto.")
                    except FileNotFoundError:
                        st.error("No se encontró Node.js. Verificá que esté instalado en el servidor.")

    st.markdown("---")
    st.info("💡 El archivo `generar_reporte.js` debe estar en la misma carpeta que `app.py`.")
