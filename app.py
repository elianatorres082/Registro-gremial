        ...
        if df_d.empty:
            ...
            with pd.ExcelWriter(buf_ger, engine="openpyxl") as writer:
                resumen_exp = pd.DataFrame({
                    ...
                    "Valor": [tot_dev, del_afect, int(reincid), sector_top]
                })
                resumen_exp.to_excel(writer, index=False, sheet_name="Resumen")
                if not sector_tasa.empty:
                    sector_df = sector_tasa.reset_index()
                    sector_df.columns = ["Sector","Desvíos"]
                    sector_df.to_excel(writer, index=False, sheet_name="Por Sector")
                if not del_dev.empty:
                    del_df = del_dev.reset_index()
                    del_df.columns = ["Delegado","Desvíos"]
                    del_df.to_excel(writer, index=False, sheet_name="Top 5 Delegados")
                if not mes_dev_sorted.empty:
                    mes_df = mes_dev_sorted.reset_index()
                    mes_df.columns = ["Mes","Desvíos"]
                    mes_df.to_excel(writer, index=False, sheet_name="Tendencia Mensual")
            buf_ger.seek(0)
            st.download_button(
