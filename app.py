    ...
    if df_all.empty:
        ...
        if df_d.empty:
            ...
            with col_izq:
                ...
                mes_dev = df_d[df_d["_es_desvio"]].groupby("_mes_dt").size()
                mes_dev = mes_dev.sort_index()
            ...
            with col_der:
                ...
                max_dev = sector_dev.max() if not sector_dev.empty else 1

                for sector, n in sector_dev.items():
                    pct_bar = int(n / max_dev * 100)
                    color_s = "#C0392B" if pct_bar >= 80 else "#E67E22" if pct_bar >= 50 else "#F39C12"
                    st.markdown(f"""
                    <div style="margin-bottom:10px;">
                        <div style="display:flex;justify-content:space-between;margin-bottom:3px;">
                            <span style="font-size:.85rem;font-weight:600">{sector}</span>
                            <span style="font-size:.85rem;font-weight:700;color:{color_s}">{n}</span>
                        </div>
                        <div style="background:#eee;border-radius:4px;height:10px;">
                            <div style="background:{color_s};width:{pct_bar}%;height:10px;border-radius:4px"></div>
                        </div>
                    </div>""", unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
        ...
        try:
            ...
            if col_legajo_fich:
                ...
                for c in df_fich_clean.columns:
                    if "entrada" in str(c).lower() or "hora" in str(c).lower():
                        col_hora = c
                        break

                # Presente = tiene hora de entrada registrada
                if col_hora:
                    df_presentes = df_fich_clean[df_fich_clean[col_hora].notna() & (df_fich_clean[col_hora].str.strip() != "")]
                else:
                    df_presentes = df_fich_clean  # si no hay columna hora, usar todos

                legajos_presentes = set(df_presentes[col_legajo_fich].str.strip().tolist())

                def estado_desde_fichada(leg):
                    return "Presente" if str(leg).strip() in legajos_presentes else "Ausente"
    ...
    # Separar presentes y sin fichada
    df_presentes_form = df_filtrado[df_filtrado["Estado"] == "Presente"].sort_values(["Turno","Nombre y Apellido"])
    df_ausentes_form  = df_filtrado[df_filtrado["Estado"] == "Ausente"].sort_values(["Turno","Nombre y Apellido"])
