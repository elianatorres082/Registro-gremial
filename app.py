...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if df_all.empty:
        ...
        tasa      = round(tot_dev / total * 100, 1) if total else 0
        reincid   = df_f[df_f["_es_desvio"]].groupby("Nombre y Apellido").size()
        reincid   = (reincid >= 3).sum()

        # ── KPIs ──
        k1,k2,k3,k4 = st.columns(4)
        color_tasa = "#C0392B" if tasa > 15 else "#E67E22" if tasa > 10 else "#27AE60"
        with k1:
            st.markdown(f'<div class="kpi-card"><div class="kpi-label">Registros del período</div><div class="kpi-value">{total}</div></div>', unsafe_allow_html=True)
        with k2:
        ...
        with k3:
            st.markdown(f'<div class="kpi-card {"danger" if tasa>15 else "warning"}"><div class="kpi-label">Tasa de Desvío</div><div class="kpi-value" style="color:{color_tasa}">{tasa}%</div></div>', unsafe_allow_html=True)
        with k4:
