...
with st.sidebar:
    ...
    pagina = st.radio(
        "", ["➕ Cargar Registro", "📋 Presencia del Día", "📊 Reportes", "📈 Dashboard Gerencial", "👥 Padrón Delegados", "📁 Ver / Editar Datos", "⬆️ Importar Histórico", "⬇️ Exportar Excel"],
...
if pagina == "➕ Cargar Registro":
    ...
    # Filtro por cuerpo gremial + selector de delegado
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if df_all.empty:
        ...
        # Intentar formatos alternativos para las que fallaron
        mask_nat = df_all["_fecha"].isna()
        if mask_nat.any():
            for fmt in ["%d/%m/%Y", "%d/%m/%y", "%Y-%m-%d", "%d-%m-%Y", "%m/%d/%Y"]:
                try:
                    df_all.loc[mask_nat, "_fecha"] = pd.to_datetime(
                        df_all.loc[mask_nat, "Fecha"], format=fmt, errors="coerce"
                    )
                    mask_nat = df_all["_fecha"].isna()
                    if not mask_nat.any():
                        break
        ...
        with fd1:
            f_desde = st.date_input("Desde", value=fecha_min, format="DD/MM/YYYY", key="rep_desde")
        with fd2:
            f_hasta = st.date_input("Hasta", value=fecha_max, format="DD/MM/YYYY", key="rep_hasta")
        ...
        st.caption("¿Qué supervisor tiene más delegados con desvíos? Para reclamar hacia adentro.")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if df_all.empty:
        ...
        df_all["_fecha"] = pd.to_datetime(df_all["Fecha"], dayfirst=True, errors="coerce")
        DESVIOS = ["No Cumple","Excede -5min","Excede 5/10 min","Excede + 10/20min",
                   "Supera + 1/2hs","SUPERA SEMANA COMPLETA","GENERA PDL"]
        ...
        if df_d.empty:
            ...
            tot_dev   = df_d["_es_desvio"].sum()
            del_afect = df_d[df_d["_es_desvio"]]["Nombre y Apellido"].nunique()
            reincid_s = df_d[df_d["_es_desvio"]].groupby("Nombre y Apellido").size()
            reincid   = int((reincid_s >= 3).sum())
            ...
            <div style="background:#1A2C5B;color:white;padding:12px 20px;border-radius:8px;margin-bottom:16px;">
                <b style="font-size:1.1rem;letter-spacing:.05em">RESUMEN EJECUTIVO — {periodo}</b>
            </div>""", unsafe_allow_html=True)

            k1,k2,k3,k4 = st.columns(4)
            with k1:
                st.markdown(f"""<div style="border-radius:10px;padding:20px;text-align:center;background:#FEE2E2;border-top:4px solid #C0392B;">
                    <div style="font-size:.7rem;color:#666;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Total Desvíos</div>
                    <div style="font-size:2.8rem;font-weight:700;color:#C0392B;line-height:1">{tot_dev}</div>
                    <div style="font-size:.75rem;color:#888;margin-top:4px">en el período</div>
                </div>""", unsafe_allow_html=True)
            with k2:
                st.markdown(f"""<div style="border-radius:10px;padding:20px;text-align:center;background:#FEF3C7;border-top:4px solid #E67E22;">
                    <div style="font-size:.7rem;color:#666;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Delegados afectados</div>
                    <div style="font-size:2.8rem;font-weight:700;color:#E67E22;line-height:1">{del_afect}</div>
                    <div style="font-size:.75rem;color:#888;margin-top:4px">con al menos 1 desvío</div>
                </div>""", unsafe_allow_html=True)
            with k3:
                color_r = "#C0392B" if reincid > 5 else "#E67E22" if reincid > 2 else "#27AE60"
                bg_r = "#FEE2E2" if reincid > 5 else "#FEF3C7" if reincid > 2 else "#D1FAE5"
                st.markdown(f"""<div style="border-radius:10px;padding:20px;text-align:center;background:{bg_r};border-top:4px solid {color_r};">
                    <div style="font-size:.7rem;color:#666;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Reincidentes</div>
                    <div style="font-size:2.8rem;font-weight:700;color:{color_r};line-height:1">{reincid}</div>
                    <div style="font-size:.75rem;color:#888;margin-top:4px">3 o más desvíos</div>
                </div>""", unsafe_allow_html=True)
            with k4:
                st.markdown(f"""<div style="border-radius:10px;padding:20px;text-align:center;background:#FEE2E2;border-top:4px solid #8E44AD;">
                    <div style="font-size:.7rem;color:#666;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Sector más crítico</div>
                    <div style="font-size:1.3rem;font-weight:700;color:#8E44AD;line-height:1.2;margin:8px 0">{sector_top}</div>
                    <div style="font-size:.75rem;color:#888">{int(df_d[df_d["_es_desvio"] & (df_d["Sector"]==sector_top)]["_es_desvio"].sum())} desvíos</div>
                </div>""", unsafe_allow_html=True)
            ...
            total_dev_periodo = mes_dev.sum() if not mes_dev.empty else 1
            mes_dev.index = [f"{MESES_ES.get(d.strftime('%B'),d.strftime('%B'))} {d.strftime('%y')}" for d in mes_dev.index]
            mes_pct = (mes_dev / total_dev_periodo * 100).round(1)

            sector_dev = df_d[df_d["_es_desvio"]].groupby("Sector").size().sort_values(ascending=False).head(5)
            total_dev_sector = sector_dev.sum() if not sector_dev.empty else 1
            ...
            with col_izq:
                ...
                st.bar_chart(mes_dev.rename("Desvíos"), color="#C0392B")

                # Cards con % por mes
                if not mes_dev.empty:
                    cols_mes = st.columns(min(len(mes_dev), 4))
                    for i, (mes, n) in enumerate(mes_dev.items()):
                        pct_m = mes_pct[mes]
                        color_m = "#C0392B" if pct_m >= 30 else "#E67E22" if pct_m >= 20 else "#27AE60"
                        with cols_mes[i % 4]:
                            st.markdown(f"""<div style="text-align:center;padding:6px;border-top:3px solid {color_m};background:#f9f9f9;border-radius:4px;margin-bottom:4px;">
                                <div style="font-size:.72rem;color:#666">{mes}</div>
                                <div style="font-weight:700;color:{color_m}">{int(n)} dev.</div>
                                <div style="font-size:.75rem;color:{color_m}">{pct_m}%</div>
                            </div>""", unsafe_allow_html=True)

                st.caption("📌 % = desvíos de ese mes sobre el total del período seleccionado")
            ...
            with col_der:
                ...
                for sector, n in sector_dev.items():
                    pct_s = round(n / total_dev_sector * 100, 1)
                    pct_bar = int(pct_s / 100 * 100)
                    color_s = "#C0392B" if pct_s >= 25 else "#E67E22" if pct_s >= 15 else "#F39C12"
                    st.markdown(f"""
                    <div style="margin-bottom:12px;">
                        <div style="display:flex;justify-content:space-between;margin-bottom:3px;">
                            <span style="font-size:.85rem;font-weight:600">{sector}</span>
                            <span style="font-size:.85rem;font-weight:700;color:{color_s}">{int(n)} dev. — {pct_s}%</span>
                        </div>
                        <div style="background:#eee;border-radius:4px;height:10px;">
                            <div style="background:{color_s};width:{pct_s}%;height:10px;border-radius:4px"></div>
                        </div>
                    </div>""", unsafe_allow_html=True)

                st.caption("📌 % = desvíos del sector sobre el total de desvíos del período seleccionado")
            ...
            for i, (nombre, n_dev) in enumerate(del_dev.items()):
                ...
                turno  = del_turno.get(nombre, "")
                sup    = del_super.get(nombre, "")
                tipos  = df_d[(df_d["Nombre y Apellido"]==nombre) & df_d["_es_desvio"]]["Movilidad Gremial x Semana x Dia"].value_counts()
                tipo_principal = tipos.index[0] if not tipos.empty else ""
                color = colors_rank[i]

                st.markdown(f"""
                <div style="display:flex;align-items:center;gap:12px;padding:12px;margin-bottom:8px;border:1px solid #eee;border-radius:8px;">
                    <div style="background:{color};color:white;font-weight:700;font-size:1.1rem;padding:10px 14px;border-radius:6px;min-width:42px;text-align:center">#{i+1}</div>
                    <div style="flex:1">
                        <div style="font-weight:700;font-size:.95rem">{nombre}</div>
                        <div style="color:#666;font-size:.8rem">{sector} · Turno {turno} · Sup: {sup}</div>
                        <div style="color:#999;font-size:.78rem;margin-top:2px">Principal: {tipo_principal}</div>
                    </div>
                    <div style="background:{color};color:white;font-weight:700;padding:8px 16px;border-radius:6px;text-align:center;min-width:80px;font-size:1.1rem">{n_dev} dev.</div>
                </div>""", unsafe_allow_html=True)
            ...
            try:
                ...
                with pd.ExcelWriter(buf_ger, engine="openpyxl") as writer:
                    pd.DataFrame({
                        "Métrica": ["Total Desvíos","Delegados Afectados","Reincidentes (≥3)","Sector más crítico"],
                        "Valor": [tot_dev, del_afect, reincid, sector_top]
                    }).to_excel(writer, index=False, sheet_name="Resumen")
                    if not sector_dev.empty:
                        sector_dev.reset_index().rename(columns={0:"Desvíos","Sector":"Sector"}).to_excel(writer, index=False, sheet_name="Por Sector")
                    if not del_dev.empty:
                        del_dev.reset_index().rename(columns={0:"Desvíos","Nombre y Apellido":"Delegado"}).to_excel(writer, index=False, sheet_name="Top Delegados")
                    if not mes_dev.empty:
                        mes_dev.reset_index().rename(columns={0:"Desvíos","_mes_dt":"Mes"}).to_excel(writer, index=False, sheet_name="Tendencia Mensual")
                buf_ger.seek(0)
                st.download_button(
                    "⬇️ Exportar a Excel",
                    data=buf_ger,
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if fichadas_file:
        try:
            ...
            # Limpiar: quedarse solo con filas que tienen legajo numérico válido
            ...
            for c in df_fich.columns:
                if "legajo" in str(c).lower():
                    ...
                    break
            if col_legajo_fich:
                # Filtrar solo filas con legajo numérico válido
                df_fich_clean = df_fich[df_fich[col_legajo_fich].notna()].copy()
                df_fich_clean = df_fich_clean[df_fich_clean[col_legajo_fich].str.strip().str.match(r"^[0-9]+$")]

                # Buscar columna de hora entrada
                col_hora = None
                for c in df_fich_clean.columns:
                    cl = str(c).lower().strip().replace(" ","")
                    if "entrada" in cl or ("hora" in cl and "salida" not in cl):
                        col_hora = c
                        break

                # Presente = tiene hora de entrada registrada (no NaN, no vacío, no "00:00")
                if col_hora:
                    mask_presente = (
                        df_fich_clean[col_hora].notna() &
                        (df_fich_clean[col_hora].astype(str).str.strip() != "") &
                        (df_fich_clean[col_hora].astype(str).str.strip() != "nan")
                    )
                    df_presentes = df_fich_clean[mask_presente]
                else:
                    df_presentes = df_fich_clean

                legajos_presentes = set(df_presentes[col_legajo_fich].str.strip().tolist())

                def estado_desde_fichada(leg):
                    return "Presente" if str(leg).strip() in legajos_presentes else "Pendiente"
                ...
                st.success(f"✅ Fichadas procesadas — **{presentes} presentes**, **{ausentes} ausentes**. Corregí abajo si alguno está de licencia o vacaciones.")
            else:
    ...
    # Separar en 3 grupos
    df_presentes_form = df_filtrado[df_filtrado["Estado"] == "Presente"].sort_values(["Turno","Nombre y Apellido"])
    df_pendientes_form = df_filtrado[df_filtrado["Estado"] == "Pendiente"].sort_values(["Turno","Nombre y Apellido"])
    df_ausentes_form  = df_filtrado[df_filtrado["Estado"].isin(["Ausente","Licencia","Vacaciones"])].sort_values(["Turno","Nombre y Apellido"])

    with st.form("form_presencia"):
        if not df_presentes_form.empty:
            ...
            for _, row in df_presentes_form.iterrows():
        ...
        if not df_pendientes_form.empty:
            st.markdown(f"**🕐 Pendiente de fichar — {len(df_pendientes_form)} delegados**")
            st.caption("Todavía no registraron entrada. Puede ser que su turno aún no empezó.")
            hp1,hp2,hp3,hp4,hp5,hp6 = st.columns([3,2,1,1,2,2])
            hp1.markdown("**Nombre**"); hp2.markdown("**Sector**"); hp3.markdown("**Turno**")
            hp4.markdown("**Cargo**"); hp5.markdown("**Estado**"); hp6.markdown("**Observación**")
            st.divider()
            for _, row in df_pendientes_form.iterrows():
                c1,c2,c3,c4,c5,c6 = st.columns([3,2,1,1,2,2])
                c1.write(row["Nombre y Apellido"])
                c2.write(row["Sector"])
                c3.write(row["Turno"])
                c4.write(row["Cargo"])
                est_idx = ESTADOS_PRESENCIA.index(row["Estado"]) if row["Estado"] in ESTADOS_PRESENCIA else 1
                estado = c5.selectbox("", ESTADOS_PRESENCIA, index=est_idx, key=f"est_{row['Legajo']}", label_visibility="collapsed")
                obs    = c6.text_input("", value=row["Observacion"], key=f"obs_{row['Legajo']}", placeholder="Opcional", label_visibility="collapsed")
                nuevos_registros.append({
                    "Legajo": row["Legajo"], "Nombre y Apellido": row["Nombre y Apellido"],
                    "Sector": row["Sector"], "Turno": row["Turno"], "Cargo": row["Cargo"],
                    "Fecha": fecha_str, "Estado": estado, "Observacion": obs,
                })

        st.markdown("---")

        if not df_ausentes_form.empty:
            st.markdown(f"**⚠️ Sin registro de fichada — {len(df_ausentes_form)} delegados**")
            st.caption("Revisá si están de licencia, vacaciones u otro motivo.")
            ha1,ha2,ha3,ha4,ha5,ha6 = st.columns([3,2,1,1,2,2])
            ha1.markdown("**Nombre**"); ha2.markdown("**Sector**"); ha3.markdown("**Turno**")
            ha4.markdown("**Cargo**"); ha5.markdown("**Estado**"); ha6.markdown("**Observación**")
            st.divider()

            for _, row in df_ausentes_form.iterrows():
                c1,c2,c3,c4,c5,c6 = st.columns([3,2,1,1,2,2])
                c1.write(row["Nombre y Apellido"])
                c2.write(row["Sector"])
                c3.write(row["Turno"])
                ...
                est_idx = ESTADOS_PRESENCIA.index(row["Estado"]) if row["Estado"] in ESTADOS_PRESENCIA else 0
                estado = c5.selectbox("", ESTADOS_PRESENCIA, index=est_idx, key=f"est_{row['Legajo']}", label_visibility="collapsed")
                obs    = c6.text_input("", value=row["Observacion"], key=f"obs_{row['Legajo']}", placeholder="Motivo...", label_visibility="collapsed")
                nuevos_registros.append({
                    "Legajo": row["Legajo"],
                    "Nombre y Apellido": row["Nombre y Apellido"],
                    "Sector": row["Sector"],
...
# ═══════════════════════════════════════════════════════════════════════════════
# PÁGINA — PADRÓN DE DELEGADOS
# ═══════════════════════════════════════════════════════════════════════════════
elif pagina == "👥 Padrón Delegados":
    st.markdown('<div class="section-title">👥 Padrón de Delegados y CIR</div>', unsafe_allow_html=True)

    # Construir DataFrame desde DELEGADOS
    df_pad = pd.DataFrame([
        {"Legajo": leg, "Nombre y Apellido": d["nombre"], "Cuerpo Gremial": d["cuerpo"],
         "Sector": d["sector"], "Cargo": d["cargo"], "Turno": d["turno"], "Reporta a": d["reporta"]}
        for leg, d in DELEGADOS.items()
    ])

    # ── KPIs ──
    total_del = len(df_pad)
    total_cir = (df_pad["Cargo"] == "CIR").sum()
    total_del_cargo = (df_pad["Cargo"] == "Delegado").sum()
    cuerpos = df_pad["Cuerpo Gremial"].nunique()

    k1,k2,k3,k4 = st.columns(4)
    with k1:
        st.markdown(f'<div class="kpi-card"><div class="kpi-label">Total Representantes</div><div class="kpi-value">{total_del}</div></div>', unsafe_allow_html=True)
    with k2:
        st.markdown(f'<div class="kpi-card ok"><div class="kpi-label">Delegados</div><div class="kpi-value">{total_del_cargo}</div></div>', unsafe_allow_html=True)
    with k3:
        st.markdown(f'<div class="kpi-card warning"><div class="kpi-label">CIR</div><div class="kpi-value">{total_cir}</div></div>', unsafe_allow_html=True)
    with k4:
        st.markdown(f'<div class="kpi-card"><div class="kpi-label">Cuerpos Gremiales</div><div class="kpi-value">{cuerpos}</div></div>', unsafe_allow_html=True)

    st.markdown("---")

    # ── Resumen por Cuerpo Gremial ──
    st.markdown("**Por Cuerpo Gremial**")
    resumen_cuerpo = df_pad.groupby("Cuerpo Gremial").agg(
        Total=("Legajo","count"),
        Delegados=("Cargo", lambda x: (x=="Delegado").sum()),
        CIR=("Cargo", lambda x: (x=="CIR").sum()),
    ).reset_index().sort_values("Total", ascending=False)

    for _, row in resumen_cuerpo.iterrows():
        st.markdown(f"""
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 16px;margin-bottom:6px;background:#f9f9f9;border-radius:8px;border-left:4px solid #2E4A9E;">
    ...
    resumen_turno = df_pad.groupby("Turno").agg(
        Total=("Legajo","count"),
        Delegados=("Cargo", lambda x: (x=="Delegado").sum()),
        CIR=("Cargo", lambda x: (x=="CIR").sum()),
    ).reset_index().sort_values("Total", ascending=False)

    t_cols = st.columns(len(resumen_turno))
    for i, (_, row) in enumerate(resumen_turno.iterrows()):
        with t_cols[i]:
            st.markdown(f"""<div style="text-align:center;padding:12px;border:1px solid #eee;border-radius:8px;">
                <div style="font-weight:700;color:#1A2C5B;font-size:1rem">{row['Turno']}</div>
    ...
    with fc1:
        fil_cuerpo = st.selectbox("Cuerpo Gremial", ["Todos"] + sorted(df_pad["Cuerpo Gremial"].unique().tolist()), key="pad_cuerpo")
    with fc2:
        fil_cargo  = st.selectbox("Cargo", ["Todos","Delegado","CIR"], key="pad_cargo")
    with fc3:
        fil_turno  = st.selectbox("Turno", ["Todos"] + sorted(df_pad["Turno"].unique().tolist()), key="pad_turno")

    df_show_pad = df_pad.copy()
    if fil_cuerpo != "Todos": df_show_pad = df_show_pad[df_show_pad["Cuerpo Gremial"] == fil_cuerpo]
    if fil_cargo  != "Todos": df_show_pad = df_show_pad[df_show_pad["Cargo"] == fil_cargo]
    if fil_turno  != "Todos": df_show_pad = df_show_pad[df_show_pad["Turno"] == fil_turno]

    st.dataframe(df_show_pad.sort_values(["Cuerpo Gremial","Nombre y Apellido"]).reset_index(drop=True), use_container_width=True, height=400)
    st.caption(f"Mostrando {len(df_show_pad)} de {total_del} representantes")
