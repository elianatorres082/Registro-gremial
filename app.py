        ...
        try:
            ...
            for c in df_fich.columns:
                if "legajo" in str(c).lower():
                    ...
                    break
            if col_legajo_fich:
                # Filtrar solo filas con legajo numérico (descartar filas ASIMRA/SMATA/encabezados)
                df_fich_clean = df_fich[df_fich[col_legajo_fich].notna()].copy()
                df_fich_clean = df_fich_clean[df_fich_clean[col_legajo_fich].str.strip().str.match(r"^\d+$")]
                legajos_fichados = set(df_fich_clean[col_legajo_fich].str.strip().tolist())

                # Cruzar con delegados — marcar presente si fichó, ausente si no
