...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    tipo_import = st.radio(
        ...
        ["Excel de Analisis (solapa Datos — formato semanal)", "Excel de registro gremial (formato estándar)"],
        key="tipo_import"
    )
    ...
    if archivo and "Analisis" in tipo_import:
        try:
            ...
            st.success(f"Archivo procesado: **{len(df_conv)} registros** — {(df_conv['Movilidad Gremial x Semana x Dia']!='Cumple').sum()} desvíos ({round((df_conv['Movilidad Gremial x Semana x Dia']!='Cumple').sum()/len(df_conv)*100,1)}% tasa)")
