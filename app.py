...
# Misma lista pero con "Cumple" primero — usada en el formulario de Cargar Registro
MOVILIDAD_OPTS_CON_CUMPLE = ["Cumple"] + MOVILIDAD_OPTS
...
def _render_seccion_externa(titulo, emoji, data_file, padron_file, key_prefix):
    ...
    with tab_rep:
        ...
        if df_rep.empty:
            ...
            # Reincidentes sobre el ultimo mes con datos, igual criterio que Reportes/Dashboard
            if not df_rf.empty and df_rf["_fecha"].notna().any():
                ultimo_mes_rf = df_rf["_fecha"].max().to_period("M")
                df_um_rf = df_rf[df_rf["_fecha"].dt.to_period("M") == ultimo_mes_rf]
                reinc = int((df_um_rf[df_um_rf["_es_desvio"]].groupby("Nombre y Apellido").size() >= 3).sum())
            else:
                reinc = 0
            ...
            st.caption("Reincidentes = 3+ desvios en el ultimo mes con datos del periodo filtrado (no se acumula con meses anteriores).")
            if tot > 0:
                st.markdown("---")
...
if pagina == "Cargar Registro":
    ...
    with f2:
        ...
        nombres_lista = ["— Selecciona un delegado —"] + [f"{leg} · {d['nombre']}" for leg, d in delegados_filtrados]
        sel = st.selectbox("Selecciona el delegado", nombres_lista, key="sel_delegado")

    # Tambien permitir escribir el legajo directamente y autocompletar al instante
    st.caption("O escribi el legajo directamente para autocompletar:")
    legajo_manual = st.text_input("Legajo (autocompleta)", key="legajo_manual_carga", placeholder="Ej: 9500")

    leg_auto, delegado_data = "", {}
    if sel != "— Selecciona un delegado —":
        leg_auto = sel.split(" · ")[0]
        delegado_data = DELEGADOS.get(leg_auto, {})
    elif legajo_manual.strip() in DELEGADOS:
        leg_auto = legajo_manual.strip()
        delegado_data = DELEGADOS.get(leg_auto, {})
    ...
    if delegado_data:
        ...
        st.warning("Legajo no encontrado en el padron — completa los datos manualmente.")
    ...
    with st.form("form_carga", clear_on_submit=True):
        ...
        with c3:
            ...
            if fecha > date.today():
                st.caption(":warning: La fecha seleccionada es futura — verifica que sea correcta.")

        st.markdown("---")
        st.markdown("**Movilidad y Excedencia**")
        mc1, mc2 = st.columns(2)
        with mc1: movilidad  = st.selectbox("Movilidad Gremial x Semana x Dia *", MOVILIDAD_OPTS_CON_CUMPLE, index=0)
        ...
        with lc2: info_lic = st.text_input("Informacion Licencia")
        with lc3: llt      = st.selectbox("LLT / Ausencia Inj.", LLT_OPTS)

        st.markdown("**Observaciones y Accion**")
        oc1, oc2, oc3 = st.columns(3)
        with oc1: obs_extra   = st.text_area("Observaciones Extras", height=80)
        with oc2: motivo_desv = st.text_area("Motivo (Detallar desvios)", height=80)
        with oc3: accion      = st.selectbox("Accion", ACCION_OPTS)

        submitted = st.form_submit_button("Guardar Registro", use_container_width=True, type="primary")

    if submitted:
        if not legajo or not nombre:
            st.error("Completa al menos Legajo y Nombre.")
        else:
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if df_all.empty:
        ...
        # Reincidentes: SIEMPRE sobre el ultimo mes calendario disponible en los datos filtrados,
        # sin importar el rango Desde/Hasta elegido arriba. Esto evita que el indicador pierda
        # sentido cuando se mira un periodo largo (ej: 8 meses) donde casi todos llegan a 3+.
        if not df_f.empty and df_f["_fecha"].notna().any():
            ultimo_mes_periodo = df_f["_fecha"].max().to_period("M")
            df_ultimo_mes = df_f[df_f["_fecha"].dt.to_period("M") == ultimo_mes_periodo]
            reincid = int((df_ultimo_mes[df_ultimo_mes["_es_desvio"]].groupby("Nombre y Apellido").size() >= 3).sum())
            mes_reincid_label = ultimo_mes_periodo.strftime("%B %Y")
        else:
            reincid = 0
            mes_reincid_label = "—"
        ...
        st.caption(f"Reincidentes = delegados con 3 o más desvíos dentro de **{mes_reincid_label}** (el último mes con datos en el período filtrado). No se acumula con meses anteriores, para que el indicador refleje el comportamiento actual y no se diluya en rangos largos.")
        ...
        jefe_df["Del. con desvíos"] = jefe_df["Del. con desvíos"].fillna(0).astype(int)
        ...
        for jefe, row in jefe_df.iterrows():
            ...
            n_del_dev = int(row["Del. con desvíos"])
            st.markdown(f"""<div style="border-left:4px solid {color_j};padding:10px 16px;margin-bottom:8px;background:#fafafa;border-radius:0 8px 8px 0;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    ...
                    <span style="color:#666;font-size:.8rem;margin-left:8px">{n_del_dev} delegados con desvíos</span></div>
        ...
        st.markdown("---")
        st.markdown(f"### Reincidentes — 3 o más desvíos en {mes_reincid_label}")
        st.caption("Se muestra solo el ultimo mes con datos del periodo filtrado arriba, no todo el rango Desde/Hasta.")
        if not df_f.empty and df_f["_fecha"].notna().any():
            dev_por_del  = df_ultimo_mes[df_ultimo_mes["_es_desvio"]].groupby("Nombre y Apellido").size().sort_values(ascending=False)
        else:
            dev_por_del = pd.Series(dtype=int)
        reincidentes = dev_por_del[dev_por_del >= 3]
        if reincidentes.empty:
            st.success(f"Sin reincidentes en {mes_reincid_label}.")
        else:
            for nombre_r, n_dev in reincidentes.items():
                grupo    = df_ultimo_mes[(df_ultimo_mes["Nombre y Apellido"] == nombre_r) & df_ultimo_mes["_es_desvio"]].sort_values("_fecha")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if df_all.empty:
        ...
        # Avisar si hay fechas futuras improbables que distorsionan el rango
        fechas_futuras = fechas_validas[fechas_validas.dt.date > date.today()]
        if not fechas_futuras.empty:
            filas_futuras = df_all[df_all["_fecha"].dt.date > date.today()][["Legajo","Nombre y Apellido","Fecha"]]
            with st.expander(f"Aviso: {len(fechas_futuras)} registro(s) con fecha futura ({fechas_futuras.max().date().strftime('%d/%m/%Y')}) — pueden estar mal cargados", expanded=False):
                st.dataframe(filas_futuras, use_container_width=True)
                st.caption("Revisa estos registros en 'Ver / Editar Datos' si la fecha es un error de tipeo.")
        ...
        if df_d.empty:
            ...
            # Reincidentes: SIEMPRE sobre el ultimo mes calendario disponible en el rango filtrado,
            # no sobre todo el periodo Desde/Hasta. Evita que el indicador pierda sentido en rangos largos.
            ultimo_mes_periodo_d = df_d["_fecha"].max().to_period("M")
            df_ultimo_mes_d = df_d[df_d["_fecha"].dt.to_period("M") == ultimo_mes_periodo_d]
            reincid = int((df_ultimo_mes_d[df_ultimo_mes_d["_es_desvio"]].groupby("Nombre y Apellido").size() >= 3).sum())
            mes_reincid_label_d = ultimo_mes_periodo_d.strftime("%B %Y")
            ...
            with k3:
                ...
                st.markdown(f"""<div style="border-radius:10px;padding:20px;text-align:center;background:{bg};border-top:4px solid {cr};">
                    ...
                    <div style="font-size:.75rem;color:#888">3+ desvíos en {mes_reincid_label_d}</div></div>""", unsafe_allow_html=True)
            with k4:
            ...
            st.caption(f"'Reincidentes' = delegados con 3 o más desvíos dentro de **{mes_reincid_label_d}** unicamente (el ultimo mes con datos del periodo filtrado). No se acumula con meses anteriores aunque el rango Desde/Hasta sea mas largo — esto evita que casi todos terminen marcados como reincidentes en periodos extensos.")
...
# ═══════════════════════════════════════════════════════════════════════════════
# PAGINA — IMPORTAR HISTORICO  (soporta multiples archivos a la vez)
# ═══════════════════════════════════════════════════════════════════════════════
elif pagina == "Importar Historico":
    st.markdown('<div class="section-title">Importar Historico desde Excel</div>', unsafe_allow_html=True)
    st.info("Podes subir uno o varios archivos juntos (por ejemplo el de Analisis y el de Bloque Gremial al mismo tiempo). Cada uno se procesa por separado y se combinan antes de confirmar.")

    archivos = st.file_uploader(
        "Selecciona uno o varios archivos Excel (.xlsx)",
        type=["xlsx","xls"],
        accept_multiple_files=True,
        key="multi_uploader_historico"
    )
    ...
    if archivos:
        ...
        for archivo in archivos:
            ...
            try:
                xl_check = pd.ExcelFile(archivo)
                sheet_names = xl_check.sheet_names
            except Exception as e:
                st.error(f"No se pudo leer {nombre_arch}: {e}")
                continue

            # Detectar tipo de archivo automaticamente
            es_analisis = "Datos" in sheet_names
            es_bloque   = any(any(mes in s.upper() for mes in
            ...
            if es_analisis:
                try:
                    ...
                    n_cum = len(df_conv) - n_dev
                    st.success(f"Detectado como **Excel de Analisis** — {len(df_conv)} registros, {n_dev} desvios, {n_cum} cumple")
                    dfs_para_combinar.append(df_conv)
                    resumen_archivos.append((nombre_arch, len(df_conv), n_dev))
                except ImportError:
                    st.error("Falta importar_analisis.py en la carpeta del repo.")
                except Exception as e:
                    st.error(f"Error procesando {nombre_arch}: {e}")

            elif es_bloque:
                try:
                    from importar_bloque import convertir_bloque_a_registro, MESES_DESDE_ABRIL
                    meses_disponibles = [m for m in MESES_DESDE_ABRIL if any(m in s.upper() for s in sheet_names)]
                    meses_sel = st.multiselect(
                        f"Meses a importar de {nombre_arch}",
                        meses_disponibles if meses_disponibles else MESES_DESDE_ABRIL,
                        default=meses_disponibles if meses_disponibles else MESES_DESDE_ABRIL,
                        key=f"meses_{nombre_arch}"
                    )
                    if meses_sel:
                        df_conv2, solapas_proc = convertir_bloque_a_registro(archivo, meses_incluir=meses_sel)
                        if df_conv2.empty:
                            st.warning(f"No se encontraron registros en {nombre_arch} para los meses seleccionados.")
                        else:
                            n_dev2 = (~df_conv2['Movilidad Gremial x Semana x Dia'].str.lower().isin(['cumple',''])).sum()
                            st.success(f"Detectado como **Bloque Gremial** — {len(solapas_proc)} solapas, {len(df_conv2)} registros, {n_dev2} desvios")
                            dfs_para_combinar.append(df_conv2)
                            resumen_archivos.append((nombre_arch, len(df_conv2), n_dev2))
                except ImportError:
                    st.error("Falta importar_bloque.py en la carpeta del repo.")
                except Exception as e:
                    st.error(f"Error procesando {nombre_arch}: {e}")

            else:
                # Formato estandar (multi-solapa generico)
                try:
                    import re as _re
                    ...
                    dfs_local, errores_local = [], []
                    for hoja in sheet_names:
                        try:
                            df_raw = xl_check.parse(hoja, header=None, dtype=str)
                            header_row = 0
                            for i in range(min(5, len(df_raw))):
                                ...
                                if any("legajo" in v or "nombre" in v for v in row_vals):
                                    header_row = i; break
                            df_h = xl_check.parse(hoja, header=header_row, dtype=str).dropna(how="all")
                            col_map = {}
                            ...
                            if not df_h.empty: dfs_local.append(df_h)
                        except Exception as e:
                            errores_local.append(f"Solapa '{hoja}': {e}")

                    for e in errores_local: st.warning(e)
                    if dfs_local:
                        df_std = pd.concat(dfs_local, ignore_index=True).fillna("")
                        n_dev3 = df_std['Movilidad Gremial x Semana x Dia'].apply(es_desvio).sum()
                        st.success(f"Detectado como **formato estandar** — {len(df_std)} registros en {len(dfs_local)} solapas, {n_dev3} desvios")
                        dfs_para_combinar.append(df_std)
                        resumen_archivos.append((nombre_arch, len(df_std), n_dev3))
                    else:
                        st.error(f"No se encontraron registros validos en {nombre_arch}.")
                except Exception as e:
                    st.error(f"Error al leer {nombre_arch}: {e}")

        # ── Resumen y confirmacion ──
        if dfs_para_combinar:
            st.markdown("---")
            st.markdown("### Resumen antes de confirmar")
            total_combinado = sum(r[1] for r in resumen_archivos)
            total_dev_combinado = sum(r[2] for r in resumen_archivos)
            for nombre_a, total_a, dev_a in resumen_archivos:
                st.markdown(f"- **{nombre_a}**: {total_a} registros, {dev_a} desvios")
            st.markdown(f"**Total combinado: {total_combinado} registros, {total_dev_combinado} desvios**")

            if st.button("Confirmar importacion de todos los archivos", type="primary", use_container_width=True):
                df_nuevo_total = pd.concat(dfs_para_combinar, ignore_index=True)
                df_actual = load_data(_mtime=_get_mtime(DATA_FILE))
                df_combinado = pd.concat([df_nuevo_total, df_actual], ignore_index=True)
                df_combinado = df_combinado.drop_duplicates(
                    subset=["Legajo","Fecha","Movilidad Gremial x Semana x Dia"], keep="last"
                )
                save_data(df_combinado)
                st.success(f"Importacion exitosa — {len(df_combinado)} registros totales en la base.")
                st.balloons()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="section-title">Padron de Delegados y CIR — SMATA Planta</div>', unsafe_allow_html=True)
    tab1, tab2 = st.tabs(["Padron Actual", "Alta / Baja"])
    df_pad = load_padron()

    with tab1:
        df_act = df_pad[df_pad["Activo"]=="Sí"]
        df_baj = df_pad[df_pad["Activo"]=="No"]
        ...
        k5.markdown(f'<div class="kpi-card danger"><div class="kpi-label">Bajas</div><div class="kpi-value">{len(df_baj)}</div></div>', unsafe_allow_html=True)
        st.caption(f"HC = Headcount: total de {len(df_act)} representantes activos en el padron (delegados + CIR). Los porcentajes de abajo son sobre este total. Bajas = personas dadas de baja del padron (no se cuentan en el HC).")
        ...
        st.markdown("---")
        st.markdown("**Por Cuerpo Gremial** — % calculado sobre el HC total activo")
        ...
        if not df_baj.empty:
            st.markdown("---")
            st.markdown(f"**Bajas registradas ({len(df_baj)})**")
            for _, r in df_baj.sort_values("Fecha Baja", ascending=False).iterrows():
                st.markdown(f"""<div style="padding:8px 14px;margin-bottom:6px;background:#FCE4E4;border-left:4px solid #C0392B;border-radius:0 6px 6px 0;font-size:.85rem">
                    <b>{r['Nombre y Apellido']}</b> <span style="color:#888">Leg. {r['Legajo']}</span><br>
                    <span style="color:#C0392B">Baja: {r.get('Fecha Baja','')}</span> · {r.get('Motivo Baja','')}</div>""", unsafe_allow_html=True)

    with tab2:
        st.markdown("### Registrar Alta de Delegado")

        # Autocompletado: si el legajo ya existe en DELEGADOS, completa los datos
        a_leg_check = st.text_input("Legajo (autocompleta si existe en el padron base)", key="a_leg_check")
        a_data_auto = DELEGADOS.get(a_leg_check.strip(), {}) if a_leg_check.strip() else {}
        if a_data_auto:
            st.success(f"Encontrado: **{a_data_auto['nombre']}** — {a_data_auto['sector']} | Turno: {a_data_auto['turno']}")

        with st.form("form_alta_smata"):
            a1,a2,a3 = st.columns(3)
            with a1:
                a_leg = st.text_input("Legajo *", value=a_leg_check.strip())
                a_nom = st.text_input("Nombre y Apellido *", value=a_data_auto.get("nombre",""))
                cuerpos_alta = sorted(set(d["cuerpo"] for d in DELEGADOS.values()))
                cb_val = a_data_auto.get("cuerpo", cuerpos_alta[0] if cuerpos_alta else "")
                cb_idx = cuerpos_alta.index(cb_val) if cb_val in cuerpos_alta else 0
                a_cb  = st.selectbox("Cuerpo Gremial", cuerpos_alta, index=cb_idx)
            with a2:
                sec_val = a_data_auto.get("sector", SECTORES_OPTS[0])
                sec_idx = SECTORES_OPTS.index(sec_val) if sec_val in SECTORES_OPTS else 0
                a_sec = st.selectbox("Sector", SECTORES_OPTS, index=sec_idx)
                car_val = a_data_auto.get("cargo", "Delegado")
                a_car = st.selectbox("Cargo", ["Delegado","CIR"], index=0 if car_val=="Delegado" else 1)
                tur_val = a_data_auto.get("turno", TURNO_OPTS[0])
                tur_idx = TURNO_OPTS.index(tur_val) if tur_val in TURNO_OPTS else 0
                a_tur = st.selectbox("Turno", TURNO_OPTS, index=tur_idx)
            with a3:
                a_rep = st.text_input("Reporta a", value=a_data_auto.get("reporta",""))
            ...
            if st.form_submit_button("Registrar Alta", type="primary", use_container_width=True):
                if not a_leg or not a_nom:
                    st.error("Completa Legajo y Nombre.")
                else:
        ...
        if df_act2.empty:
            ...
            sel_baja = st.selectbox("Selecciona el delegado a dar de baja",
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    df_fdp = _load_fdp()
    # Deduplicar por legajo+nombre (puede haber quedado un duplicado de cargas anteriores)
    if not df_fdp.empty:
        antes = len(df_fdp)
        df_fdp = df_fdp.drop_duplicates(subset=["legajo","nombre"], keep="last").reset_index(drop=True)
        if len(df_fdp) < antes:
            _save_fdp(df_fdp)
    ...
    fallecido_fdp  = df_fdp[df_fdp["status"] == "Fallecimiento"]
    renuncia_fdp   = df_fdp[df_fdp["status"] == "Renuncia Sindicato"]

    k1,k2,k3,k4,k5,k6 = st.columns(6)
    k1.markdown(f'<div class="kpi-card"><div class="kpi-label">Total Historico</div><div class="kpi-value">{len(df_fdp)}</div></div>', unsafe_allow_html=True)
    ...
    k5.markdown(f'<div class="kpi-card warning"><div class="kpi-label">Cargo Politico</div><div class="kpi-value">{len(politic_fdp)}</div></div>', unsafe_allow_html=True)
    k6.markdown(f'<div class="kpi-card"><div class="kpi-label">Fallecimiento</div><div class="kpi-value">{len(fallecido_fdp)}</div></div>', unsafe_allow_html=True)

    tab_act, tab_hist, tab_edit = st.tabs(["Activos Hoy", "Historial Completo", "Editar Padron"])
    ...
    with tab_hist:
        ...
        fil_st = st.selectbox("Filtrar por Status",
            ["Todos","Activo","Desvinculado","Reincorporado a TASA","Cargo Político","Fallecimiento","Renuncia Sindicato"],
            key="fdp_fil_st")
        df_fdp_show = df_fdp if fil_st == "Todos" else df_fdp[df_fdp["status"] == fil_st]
        df_fdp_show = df_fdp_show.sort_values("desde")
        if df_fdp_show.empty:
            st.info("No hay registros con ese status.")
        ...
        buf_fdp.seek(0)
        st.download_button("Exportar a Excel", data=buf_fdp, file_name="smata_fuera_de_planta.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    with tab_edit:
        st.markdown("### Agregar registro al padron")
        ...
        with st.form("form_fdp_alta"):
            ...
            with e3:
                ...
                n_det  = st.text_input("Detalle / Observacion")
            if st.form_submit_button("Agregar", type="primary", use_container_width=True):
                if not n_leg or not n_nom or not n_carg:
                    st.error("Completa Legajo, Nombre y Cargo Gremial.")
                elif n_leg.strip() in df_fdp["legajo"].values:
                    st.error(f"Ya existe un registro con el legajo {n_leg.strip()}. Usa 'Cambiar status' para editarlo.")
                else:
        ...
        st.markdown("---")
        st.markdown("### Cambiar status de una persona")
        if df_fdp.empty:
            st.info("No hay registros en el padron.")
        else:
            opciones_edit = [f"{r['legajo']} · {r['nombre']}" for _, r in df_fdp.iterrows()]
            sel_edit_fdp = st.selectbox("Selecciona", opciones_edit, key="fdp_edit_sel")
            ...
            st.caption("Usa esto solo si cargaste un registro por error.")
            sel_del_fdp2 = st.selectbox("Selecciona el registro a eliminar", opciones_edit, key="fdp_del_sel2")
            idx_del_fdp2 = opciones_edit.index(sel_del_fdp2)
            colx1, colx2 = st.columns([4,1])
            with colx2:
                if st.button("Eliminar", type="primary", key="fdp_del_btn_final"):
                    nombre_del = df_fdp.iloc[idx_del_fdp2]["nombre"]
                    df_fdp = df_fdp.drop(df_fdp.index[idx_del_fdp2]).reset_index(drop=True)
                    _save_fdp(df_fdp)
                    st.success(f"{nombre_del} eliminado del padron.")
                    st.rerun()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    # Mandato — solo SMATA Planta
    st.markdown("**Mandato vigente**")
    m_info_planta = MANDATOS.get("SMATA Planta", {})
    if m_info_planta.get("inicio"):
        st.markdown(f"""<div style="background:#EEF2FF;border-radius:8px;padding:10px 14px;margin-bottom:10px;max-width:400px;">
            <div style="font-weight:700;color:#2E4A9E;font-size:.9rem">SMATA Planta</div>
            <div style="font-size:.8rem;color:#444">Mandato {m_info_planta['periodo']}</div>
            <div style="font-size:.78rem;color:#666">Inicio: {m_info_planta['inicio']} · Fin est.: {m_info_planta['fin_estimado']}</div>
        </div>""", unsafe_allow_html=True)
    st.markdown("---")
    ...
    # ── TAB: SMATA Planta ──
    with tab_planta:
        st.markdown("### Movimientos en el padron SMATA Planta")

        fil_mov = st.radio("Ver", ["Todos los movimientos", "Solo Altas", "Solo Bajas"], horizontal=True, key="hist_fil_mov")

        altas  = df_pad[df_pad["Fecha Alta"].str.strip().ne("") & df_pad["Activo"].eq("Sí")]
        bajas  = df_pad[df_pad["Activo"]=="No"]

        if fil_mov != "Solo Bajas":
        ...
        if fil_mov != "Solo Altas":
            ...
            if not bajas.empty:
                ...
                st.info("Sin bajas registradas en el padron SMATA Planta.")

    # ── TAB: SMATA FDP ──
    with tab_fdp:
        st.markdown("### Evolucion SMATA Fuera de Planta (2008 — hoy)")
