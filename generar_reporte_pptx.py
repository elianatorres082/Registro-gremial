...
def generar_reporte_bytes(df, periodo_label="", n_delegados=None):
    ...
    kpis = [
        ...
        ("Registros Cumple", f"{n_cumple:,}".replace(",","."), fmt_pct(100-D["tasa"])+" del total", GREEN),
    ]
    kw = 2.1
    ...
    # Tendencia en texto
    add_text(s2, "EVOLUCION MENSUAL", 0.35, 2.45, 9, 0.25, size=8, bold=True, color=NAVY)
    if D["tendencia"]:
        col_w_t = 9.0 / len(D["tendencia"])
        for i, t in enumerate(D["tendencia"]):
            tc = RED if t["pct"]>=20 else ORANGE if t["pct"]>=12 else GREEN
            add_rect(s2, 0.35+i*col_w_t, 2.75, col_w_t-0.08, 0.08, tc)
            bar_h = min(t["pct"]/30 * 1.3, 1.3)
            add_rect(s2, 0.35+i*col_w_t, 2.75+1.3-bar_h, col_w_t-0.08, bar_h, tc)
            add_text(s2, t["label"][:3], 0.35+i*col_w_t, 4.1, col_w_t-0.08, 0.25,
                     size=7.5, color=GRAY, align=PP_ALIGN.CENTER)
            add_text(s2, fmt_pct(t["pct"]), 0.35+i*col_w_t, 4.35, col_w_t-0.08, 0.25,
